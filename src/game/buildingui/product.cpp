    ...
    coinIcon(util::AssetLoader::get().coinIcon), coinText(util::AssetLoader::get().font)
{
    ...
    coinText.setPosition({coinIcon.getPosition().x + coinIcon.getGlobalBounds().size.x * 1.2f, position.y + background.getSize().y / 2.f - coinText.getGlobalBounds().size.y / 2.f});
