...
template < typename T >
inline void read(tinyxml2::XMLElement *, T *);

template < >
inline void read<double>(tinyxml2::XMLElement * elem, double * d) { elem->QueryDoubleText(d); }

template < >
inline void read<int>(tinyxml2::XMLElement * elem, int * i) { elem->QueryIntText(i); }

template < >
inline void read<std::string>(tinyxml2::XMLElement * elem, std::string * str) { *str = elem->GetText(); }

/* --------------------------------------------------------------------------------------- */
template < typename T >
inline void load(tinyxml2::XMLElement * , std::string_view , T & );
